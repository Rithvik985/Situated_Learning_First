...
from Question_Extractor.src.verification import verify_and_process_output_directory
from Question_Extractor.src.connection import extract_from_pdf
import fitz  # PyMuPDF      
...
def process_single_pdf_with_verification(pdf_path, model="ibnzterrell/Meta-Llama-3.3-70B-Instruct-AWQ-INT4", 
    ...
    if not output_files:
        print("No question PDFs created. Exiting.")
        return ""

    output_dir = os.path.dirname(output_files[0])
    print(f"\nSTEP 2: Verifying extracted questions in: {output_dir}")
    print("=" * 70)

    results = verify_and_process_output_directory(output_dir, model, base_url)
    valid_pdfs = results.get("valid_files", []) if results else []

    print(f"\nExtracting text from {len(valid_pdfs)} valid PDFs...")
    all_extracted_text = ""

    for valid_pdf in valid_pdfs:
        print(f"  - {valid_pdf}")
        try:
            extracted = extract_from_pdf(valid_pdf, output_dir=os.path.dirname(valid_pdf))
            all_extracted_text += extracted + "\n\n"
        except Exception as e:
            print(f"Error extracting from {valid_pdf}: {e}")
